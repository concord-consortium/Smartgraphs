...
/** @scope Smartgraphs.GraphView.prototype */ {
  ...
  titleView: SC.LabelView.design(Smartgraphs.PointerEventsSupport, {
    pointerEvents: 'none',
  ...
  graphCanvasView: RaphaelViews.RaphaelCanvasView.design({
    ...
    mouseExited: function (evt) {
      this.getPath('axesView.inputAreaView')._unsetCurrentPointerCoordinates();
    },
    ...
    axesView: RaphaelViews.RaphaelView.design({
      ...
      // Event listeners are here, instead of on the inputAreaView, so that the gridView doesn't
      // obstruct events.
    ...
    // Holds the 'overlay annotations'; is transparent to mouse and touch events.
    overlayAnnotationsHolder: RaphaelViews.RaphaelView.design(Smartgraphs.PointerEventsSupport, {
      pointerEvents: 'none'
    }),
  ...
  legendView: SC.View.extend({
      ...
      didLayoutChange: function () {
        var layout = { top: this.initialTopPos, left: this.initialLeftPos, width: this.maxWidth, height: this.maxHeight, zIndex: 200 };
